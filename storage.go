...
func (si *studentInfo) writeSemesterInfo(md *mkdown.Markdown) {
	...
	for _, semester := range si.semesters {
		...
		text := mkdown.NewText(fmt.Sprintf("**所选学分**:%v\t**所获学分**:%v\t**重修学分**:%v", semester.selectedCredit, semester.gainCredit, semester.retakeCredit))
		...
		for _, course := range semester.courses {
			table.Add(course.courseCode).Add(course.collegeName).Add(course.courseNature).Add(course.belong).Add(course.credit).Add(course.gradePoint)
